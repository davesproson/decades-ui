...
import { DashboardOptions, DashboardProps } from "./dashboard.types";
...
const LimitSetter = (props: {
        show: boolean, onHide: ()=>void, name: string, minVal: number, maxVal: number,
        useURL: boolean
    }) => {
    ...
    const setUrlLimits = ({ name, min, max }: {name: string, min: number, max: number}) => {

        const urlLimits = searchParams.getAll('limits').filter(x => x.split(',')[0] !== props.name)

        const urlMin = isNaN(min) ? '' : min?.toString()
        const urlMax = isNaN(max) ? '' : max?.toString()
        urlLimits.push(`${name},${urlMin},${urlMax}`)
    ...
    const setMax = () => {
        ...
        setUrlLimits({ name: props.name, min: props.minVal, max: parseFloat(maxRef.current.value) })
    }

    const setMin = () => {
        if(!minRef.current) return
        setUrlLimits({ name: props.name, min: parseFloat(minRef.current.value), max: props.maxVal })
    }
...
interface DashPanelProps {
    ...
    setMaximized: React.Dispatch<React.SetStateAction<string|null>>,
    useURL: boolean
}
/**
 ...
 * )
 */
const LargeDashPanel = (props: DashPanelProps) => {
    ...
    if (dataVal?.length) {
        dataValString = dataVal[0]?.toFixed(2)
    } else {
        dataValString = 'No Data'
        units = ''
    }
    ...
    const [minValid, maxValid] = validLimit
        ? [validLimit.min, validLimit.max]
        : [NaN, NaN]

    const inAlarm = (parseFloat(dataValString) < minValid) || (parseFloat(dataValString) > maxValid)
        ...
        <div>
            <div className={className} style={style}>
                {filteredParams.map(x => <DashPanel size={size}
                    key={x.ParameterName}
                    param={x}
                    value={data ? data[x.ParameterName] : null}
